...
import type { UserAppRelease } from '../types/api'
...
export function UserAppReleasePage() {
  ...
  const latestQuery = useQuery({
    queryKey: ['admin', 'user-app-releases', 'latest'],
    queryFn: adminApi.getLatestUserAppRelease,
    retry: false,
  })

  const listQuery = useQuery({
    queryKey: ['admin', 'user-app-releases', 'list'],
    queryFn: () => adminApi.listUserAppReleases(1, 20),
  })
  ...
  const uploadMutation = useMutation({
    mutationFn: () => {
      ...
      return adminApi.uploadUserAppRelease({
        file,
    ...
    onSuccess: (release) => {
      toast.success(`User APK v${release.version} uploaded`)
      setVersion('')
      setVersionCode('')
      setNotes('')
      setFile(null)
      if (fileRef.current) fileRef.current.value = ''
      qc.invalidateQueries({ queryKey: ['admin', 'user-app-releases'] })
    },
  ...
  return (
    <div>
      <PageHeader
        title="User app"
        description="Upload customer Android APK builds to the server (VPS disk) and share a download link with testers. Cloudinary is not used here — free plans cap files at 10MB and APKs are larger."
      />
      ...
      <div className="mb-6 grid gap-4 lg:grid-cols-2">
        <Card>
          ...
          <p className="mb-4 text-xs text-(--text-muted)">
            Testers install from this link on Android (allow install from unknown sources).
          </p>
          ...
          {latest && (
            <div className="space-y-3">
              ...
              <p className="break-all text-xs text-(--text-muted)">
                Always-latest:{' '}
                {latest.apkUrl.replace(/\/[^/]+\/download$/, '/latest/download')}
              </p>
            </div>
          )}
        </Card>
      ...
      <Card>
        ...
        <div className="space-y-2">
          {releases.map((release: UserAppRelease) => (
            <div
              key={release.id}
